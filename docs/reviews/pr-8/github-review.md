...
Recommendation: **REQUEST_CHANGES**. Commit: `21724ce35c6a4b3f5d89db265517bf77a38d425f`.

Request changes. The Python 3.12 suite passes (453 tests, 96.72% coverage), but independent probes reproduce lookahead through future risk-free rates, omitted initial drawdowns, and incorrect daily returns after dropping gaps. CAPM and target backtests are unusable through their exposed CLI options. Please fix the findings below, incorporate the foundation corrections already on local main, and resolve the documented spec discrepancies before treating 0002 as ready for v1.0.0. This review also checks packaging: PyPI/TestPyPI currently return 404 for sobres and RELEASE_ENABLED is false. Homebrew requires a separate formula/tap; publishing to PyPI does not create one. Full local report includes reproductions, performance results, release criteria, and self-test instructions.

## [P1] Use only risk-free observations available before each rebalance

Inline: `src/sobres/cli/commands/optimize.py:479`, RIGHT side.
...
## [P1] Include starting capital in the drawdown high-water mark

Inline: `src/sobres/core/risk.py:109`, RIGHT side.
...
## [P1] Preserve return intervals when dropping missing prices

Inline: `src/sobres/cli/commands/optimize.py:140`, RIGHT side.
...
## [P2] Provide the benchmark required by the exposed CAPM estimator

Inline: `src/sobres/cli/commands/optimize.py:173`, RIGHT side.
...
## [P2] Make target objectives usable in the backtest interface

Inline: `src/sobres/cli/commands/optimize.py:409`, RIGHT side.
...
## [P2] Condition or reject covariance at the public optimizer boundary

Inline: `src/sobres/core/optimize.py:184`, RIGHT side.
...
## [P2] Reject non-finite weights and invalid seeds before fetching data

Inline: `src/sobres/cli/commands/optimize.py:542`, RIGHT side.
...
## [P2] Return the requested number of frontier portfolios

Inline: `src/sobres/core/optimize.py:397`, RIGHT side.
