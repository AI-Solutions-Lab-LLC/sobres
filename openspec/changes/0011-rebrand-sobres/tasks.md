...
- [x] **C1. Docker** *(landed with 0005)* — image name `aisolutionslab/sobres`, volume examples, and
      the health check calling `sobres doctor`. → test: `sobres deploy check`
      output contains no `quantfolio`.
- [x] **C2. Landing page** *(landed with 0006)* — base URL, install snippet, and every command example.
