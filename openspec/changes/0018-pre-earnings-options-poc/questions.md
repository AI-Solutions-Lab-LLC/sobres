...
| Q5 | What are separate monthly caps for hosting and market data, and an acceptable one-time historical-data purchase? | Hosting target $0–5, planning ceiling $10; data budget undecided | Purchases / deployment |
...
| Q8 | Does “new model” mean a validated promoted version, every retraining run, or a generated portfolio? Who may promote it? | Owner promotion after backtest gates; record one model-change event per promotion/rollback, not per fit | Model lifecycle |
...
| Q12 | For which options/earnings data vendors can the owner obtain API keys or complete API documentation with sample payloads (at least two), and do the samples carry historical bid/ask with timestamps, contract identity and earnings confirmation times? | **Decided 19 September 2026: Massive and ThetaData.** Keys or complete documentation with sample payloads are still to be obtained for both, and the capability matrix recorded here before A2 (human task H1) | Provider port design |
